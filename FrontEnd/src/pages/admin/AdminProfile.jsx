...
import { FaUser, FaPhone, FaMapMarkerAlt, FaEnvelope, FaSave, FaLock, FaKey, FaEye, FaEyeSlash } from "react-icons/fa";
...
const AdminProfile = () => {
    ...
    const loadProfile = async () => {
        try {
            const response = await API.get(`/users/${user.entityId}`);
    ...
    const handleSubmit = async (e) => {
        ...
        if (!phoneNumber || !/^[6-9]\d{9}$/.test(phoneNumber)) { toast.error("Enter valid 10-digit phone number"); return; }
        if (!address || address.length < 5) { toast.error("Address must be at least 5 characters"); return; }

        setSaving(true);
        try {
            const updated = { ...profile, fullName, phoneNumber, address, gender };
            const response = await API.put(`/users/update/${user.entityId}`, updated);
            setProfile(response.data);
    ...
    const handlePwChange = async (e) => {
        ...
        try {
            ...
            // Step 2: Update the user's password in the profile
            const updated = { ...profile, password: newPassword };
            await API.put(`/users/update/${user.entityId}`, updated);
    ...
    if (loading) {
        return <AdminLayout><div className="text-center py-5 text-muted">Loading profile...</div></AdminLayout>;
    }

    const initials = profile?.fullName?.[0]?.toUpperCase() || "A";
                ...
                <Col md={4}>
                    <Card style={{ textAlign: "center" }}>
                        <Card.Body style={{ padding: "32px 24px" }}>
                            <div className="avatar-circle" style={{ background: "#4f46e5" }}>{initials}</div>
                            <h5 style={{ fontWeight: 700, marginBottom: 4 }}>{profile?.fullName || "—"}</h5>
                            <p style={{ color: "#64748b", fontSize: "0.85rem", marginBottom: 16 }}>{profile?.email}</p>
                            <Badge bg="danger" style={{ marginBottom: 20 }}>Administrator</Badge>
                            ...
                            <div style={{ textAlign: "left", borderTop: "1px solid #e2e8f0", paddingTop: 20, marginBottom: 20 }}>
                                {[
                                    ...
                                    <div key={item.label} style={{ display: "flex", gap: 12, marginBottom: 14, alignItems: "flex-start" }}>
                                        <div style={{ width: 32, height: 32, background: "#fee2e2", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", color: "#dc2626", fontSize: "0.8rem", flexShrink: 0 }}>
                                            {item.icon}
                                        </div>
                                        <div>
                            ...
                            <form onSubmit={handleSubmit}>
                                <Row className="g-3">
                                    <Col md={6}>
                                        ...
                                        <input ref={fullNameRef} defaultValue={profile?.fullName || ""} placeholder="Enter full name" className="form-control" />
                                    </Col>
                                    <Col md={6}>
                                        <Form.Label>
                                            <FaPhone style={{ marginRight: 6, color: "#10b981" }} />Phone Number
                                        </Form.Label>
                                        <input ref={phoneRef} defaultValue={profile?.phoneNumber || ""} placeholder="10-digit phone number" className="form-control" />
                                    </Col>
                                    <Col md={6}>
                                        <Form.Label>
                                            <FaUser style={{ marginRight: 6, color: "#f59e0b" }} />Gender
                                        </Form.Label>
                                        <select ref={genderRef} defaultValue={profile?.gender || ""} className="form-select">
                                    ...
                                    <Col md={6}>
                                        ...
                                        <input value={profile?.email || ""} readOnly disabled className="form-control" />
                                    </Col>
                                    <Col md={12}>
                                        <Form.Label>
                                            <FaMapMarkerAlt style={{ marginRight: 6, color: "#ef4444" }} />Address
                                        </Form.Label>
                                        <textarea ref={addressRef} defaultValue={profile?.address || ""} placeholder="Enter your full address" rows={3} className="form-control" />
                                    </Col>
                                </Row>
